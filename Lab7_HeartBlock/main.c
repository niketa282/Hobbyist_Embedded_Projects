...
// Outputs: None
void WaitForASLow(void){
SW1 = GPIO_PORTF_DATA_R&0x10; //read data from PF4 into switch1
while(SW1) //when switch1 not pressed that means AS is high, continue looping till it becomes low
{
...
void WaitForASHigh(void){
	SW1 = GPIO_PORTF_DATA_R&0x10; 
	while(!SW1)//when switch1 pressed that means AS is low, continue looping till it becomes high
	{
...
void SetVT(void){
 GPIO_PORTF_DATA_R=GPIO_PORTF_DATA_R|0x02; //PF1=0x02
}
...
// Notes:   friendly means it does not affect other bits in the port
void ClearVT(void){
  GPIO_PORTF_DATA_R=GPIO_PORTF_DATA_R&(~0x02);//PF1=0x00
}
...
// Notes:   friendly means it does not affect other bits in the port
void SetReady(void){
 GPIO_PORTF_DATA_R|=0x08;//same as saying 
}
