...
unsigned long sw1,sw2,current_value;
int main(void){  
 ...
 // SysTick_Init(); // initialize SysTick, runs at 16 MHz
  ...
  //last = NVIC_ST_CURRENT_R;
	last=GPIO_PORTF_DATA_R&0x13;
  EnableInterrupts(); // enable interrupts for the grader
  while(1){
		sw1=GPIO_PORTF_DATA_R&0x10;//read in values for both switches,implement negative logic
      ...
      Led = Led^0x02;            // toggle red LED
			...
			GPIO_PORTF_DATA_R&= ~(0x02);//led cleared i.e turned off
		}
		now=GPIO_PORTF_DATA_R&0x13;
